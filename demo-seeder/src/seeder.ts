...
async function registerUsers(count: number, emit: EmitFn): Promise<RegisteredUser[]> {
  ...
  for (let i = 0; i < count; i++) {
    ...
    try {
      await client.query(
        `INSERT INTO user_profiles (user_id, first_name, last_name, date_of_birth, fitness_goals, preferred_class_types, profile_photo_data, profile_photo_mime_type, emergency_contact_name, emergency_contact_phone, onboarding_completed_at)
         VALUES ($1, $2, $3, $4, $5::jsonb, $6::jsonb, $7, $8, $9, $10, '2026-01-01T00:00:00Z'::timestamptz)
