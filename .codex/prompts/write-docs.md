# Prompt: write-docs {feature-slug}

Full GymFlow documentation pipeline for: {ARGUMENTS}

Runs Stages 1 → 2 → 2.5 in sequence. Each stage must complete successfully
before the next begins. If a stage has open blocking questions, surface them
to the user and wait for answers before continuing.
...
If too vague, stop and ask the user to clarify.
