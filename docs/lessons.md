...
Rule: Never author a design spec or HTML prototype locally. The canonical tokens, voice, and visual rules live in `docs/design-system/` (README.md, colors_and_type.css, tailwind.gymflow.cjs, assets/). Per-feature mocks land at `docs/design-system/handoffs/{slug}/` produced in the Claude Design project — package contains `README.md` (the spec) and `design_reference/` (prototype bundle). If a handoff is missing when `/deliver` or `/redesign` runs, STOP and ask the user to produce it — do not fabricate one to unblock the pipeline. SA, developer, and reviewer all read the handoff + `docs/design-system/README.md` + `colors_and_type.css`.
