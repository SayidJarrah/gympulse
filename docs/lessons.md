...
Rule: Per-feature mocks land at `docs/design-system/handoffs/{slug}/`. **Claude Design is the preferred author.** When Claude Design quota is exhausted or the user wants a quick in-repo iteration, invoke the native `designer` agent — never fabricate a handoff inline inside another agent and never skip the handoff step. Both sources must produce output in the same shape: `README.md` spec + `design_reference/` prototype bundle. If `/deliver` or `/redesign` runs without a handoff AND the `designer` agent has not been invoked, STOP and ask the user which source to use. SA, developer, and reviewer all read the handoff + `docs/design-system/README.md` + `colors_and_type.css`.
Applies when: Running `/deliver`, `/redesign`, or `/audit`; dispatching solution-architect, developer, or reviewer for UI work; deciding whether to wait for Claude Design quota or use the native `designer` agent; any time a stale reference to `docs/design/` (without `-system`) is encountered.
