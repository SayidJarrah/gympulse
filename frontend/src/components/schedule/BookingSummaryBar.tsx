...
export function BookingSummaryBar({
  ...
  // Change 2 — always render; show placeholder when empty
  if (entries.length === 0) {
    return (
      <section className="rounded-[24px] border border-gray-800 bg-gray-900/80 px-5 py-3 shadow-md shadow-black/30">
        <p className="text-sm text-gray-500">No classes booked yet in this view.</p>
      </section>
    )
  }
    ...
    <section className="rounded-[24px] border border-gray-800 bg-gray-900/80 p-5 shadow-md shadow-black/30">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div className="space-y-1">
          {/* Change 7 — BOOKING SUMMARY overline keeps uppercase tracking (section label) */}
        ...
        {entries.slice(0, 3).map((entry) => (
          ...
          >
            <div className="flex items-start justify-between gap-3">
              ...
              {/* Change 7 — sentence case for mini-card status chip */}
              <div className="rounded-full border border-green-500/30 bg-green-500/10 px-2.5 py-1 text-xs font-medium text-green-300">
